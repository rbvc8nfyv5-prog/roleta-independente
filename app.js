...
/* ============================================================
   ...
   - SE UMA LINHA DER LOSS, ESSA LINHA CONGELA A JOGADA DELA.
   - SOMENTE ESSA LINHA USA A MESMA JOGADA NO PRÓXIMO RESULTADO.
   - AS OUTRAS LINHAS CONTINUAM GERANDO SUAS JOGADAS NORMALMENTE.
   - GREEN NO RESULTADO SEGUINTE DA LINHA CONGELADA = G1.
   - APÓS O G1, ESSA LINHA É LIBERADA.
...
if(x.timelines){
if(Array.isArray(x.timelines.AUTO))estado.timelines.AUTO=x.timelines.AUTO.slice(-MAX_TIMELINE);
["NORMAL","DINAMICO"].forEach(motor=>{
if(!x.timelines[motor])return;
RX_LIST.forEach(rx=>{
if(Array.isArray(x.timelines[motor][rx]))
estado.timelines[motor][rx]=x.timelines[motor][rx].slice(-MAX_TIMELINE);
});
});
}
...
janela.forEach(n=>{
const t=terminal(n);
if(trio.includes(t)){cobertura++;return;}
if(trio.some(c=>t===terminalAnterior(c)||t===terminalSeguinte(c)))cobertura++;
});
...
pctAlternancia:janela.length>1?alternancias/(janela.length-1)*100:0
};
}
...
const contagem={ZERO:0,VOISINS:0,ORPHELINS:0,TIERS:0};
...
if(seq[i-1]&&seq[i])transicoes[seq[i-1]][seq[i]]++;
}
...
atual,
repeticaoAtual,
pctAlternancia:seq.length>1?alternancias/(seq.length-1)*100:0
};
}
...
const estrutural=mapa&&mapa.has(numero)?mapa.get(numero):.55;
...
if(peso)calor.set(alvo,(calor.get(alvo)||0)+peso*recencia);
});
});
...
const score=frequencia*.35+fisica*.45+momento*.20;
...
const mapa=MAPA_DUZIA_FISICA[d];
const estrutural=mapa&&mapa.has(numero)?mapa.get(numero):.48;
...
return item.score*.44+estrutural*.22+calorNorm*.22+proximidade*.12;
}
...
if(peso)mapa.set(alvo,(mapa.get(alvo)||0)+peso*recencia);
});
});
...
for(let i=0;i<base.length;i++)eventos[i]=eventoNumero(base[i]);
return{base,eventos};
}
...
const forma=Math.max(0,1-erro/maxErro)*100;
...
function avaliarCentroComGiradas(centroOriginal,qtd,freq,momento,diagnostico){
let melhor=null;
...
const custoGirada=Math.abs(offset)*.15*Math.max(1,rx.suporte);
...
function gerarCandidatos(qtd,freq,momento,diagnostico){
const mapa=new Map();
...
if(!existente||c.score>existente.score)mapa.set(c.centro,c);
});
...
function montarJogada(raioX,base,diagnostico=null){
...
const candidatos2=gerarCandidatos(2,freq,momento,diagnostico);
const candidatos1=gerarCandidatos(1,freq,momento,diagnostico);
...
blocos2.forEach(b=>b.numeros.forEach(n=>numeros.add(n)));
blocos1.forEach(b=>b.numeros.forEach(n=>numeros.add(n)));
...
function gerarConfigNormal(base,rxTam,diagnostico=null){
base=limitar35(base);
...
const jogada=montarJogada(raioX,base,diagnostico);
...
function gerarConfigDinamico(base,rxTam,diagnostico=null,offset=offsetCentroAtual){
base=limitar35(base);
...
let jogada=montarJogada(raioX,base,diagnostico);
jogada=aplicarOffsetGlobalNaJogada(jogada,offset);
...
return{green:false,tipo:"FORA",lado:0};
}
...
if(!melhor)return{green:false,tipo:"FORA",lado:0};
...
return arr.filter(x=>!x.semJogada);
}

function taxa(arr,qtd){
const v=validos(arr).slice(-qtd);
if(!v.length)return 0;
return v.filter(x=>x.green).length/v.length*100;
}

const todosValidos=validos(lista);
let loss=0;
...
const cfg=gerarConfigNormal(passado,rxTam,diag);
...
const r=classificarJogada(resultado,cfg.jogada);
...
function backtestDinamico(base,rxTam,offsetAtual){
base=limitar35(base);
const timeline=[];
...
const passado=base.slice(0,i);
const diag=diagnosticarLosses(timeline);
const cfg=gerarConfigDinamico(passado,rxTam,diag,offsetAtual);
...
function pontuarConfiguracao(cfg,bt,live){
if(!cfg||!cfg.valido)return-Infinity;
...
const timeline=estado.timelines.NORMAL[rx];
const diagnostico=diagnosticarLosses(timeline);
const live=statsTimeline(timeline);

const cfg=gerarConfigNormal(base,rx,diagnostico);
if(!cfg.valido)return null;

const bt=backtestNormal(base,rx);

return Object.assign({},cfg,{
backtest:bt,
live,
score:pontuarConfiguracao(cfg,bt,live)
});
}

function analisarDinamico(base,rx){
const timeline=estado.timelines.DINAMICO[rx];
const diagnostico=diagnosticarLosses(timeline);
const live=statsTimeline(timeline);

const cfg=gerarConfigDinamico(base,rx,diagnostico,offsetCentroAtual);
if(!cfg.valido)return null;

const bt=backtestDinamico(base,rx,offsetCentroAtual);
...
RX_LIST.forEach(rx=>{
normal[rx]=analisarNormal(base,rx);
dinamico[rx]=analisarDinamico(base,rx);
});

return{normal,dinamico};
}
...
RX_LIST.forEach(rx=>{
if(seis.normal[rx]&&seis.normal[rx].valido)candidatos.push(seis.normal[rx]);
if(seis.dinamico[rx]&&seis.dinamico[rx].valido)candidatos.push(seis.dinamico[rx]);
});
...
return seis.dinamico[estado.manualRX]||null;

return seis.normal[estado.manualRX]||null;
}

function configAtiva(seis,auto){
if(estado.modo==="AUTO")return auto;
...
offsetAplicado:Number.isInteger(config.offsetAplicado)?config.offsetAplicado:0,
...
motor:c.motor,
rx:c.rx,
usarOffset:c.usarOffset,
offsetAplicado:c.offsetAplicado,
centros2:c.jogada.blocos2.map(x=>x.centro),
centro1:c.jogada.blocos1[0]?c.jogada.blocos1[0].centro:null
};
}
...
blocos2:(p.centros2||[]).map(c=>({
centro:c,
qtd:2,
numeros:setor(c,2)
})),

blocos1:p.centro1!==null&&p.centro1!==undefined
?[{
centro:p.centro1,
qtd:1,
...
const j=jogadaDeSnapshot(p);
if(!j)return{green:false,tipo:"FORA",lado:0};
return classificarJogada(numero,j);
}
...
function prepararPendenteLinha(config,freeze){
if(freeze){
const s=snapshot(freeze);
if(s)s.assinatura=assinatura();
return s;
}
...
estado.pendentes.NORMAL[rx]=prepararPendenteLinha(
seis.normal[rx],
...
estado.pendentes.DINAMICO[rx]=prepararPendenteLinha(
seis.dinamico[rx],
...
function avaliarLinhaComG1(numero,p,lista,freeze){
const sig=assinatura();
...
/* JÁ ESTAVA CONGELADA = ESTE RESULTADO É G1 */
if(freeze){
lista.push({
resultado:numero,
semJogada:false,
...
/* LOSS DE PRIMEIRA: CONGELA SÓ ESTA LINHA */
lista.push({
resultado:numero,
semJogada:false,
green:false,
...
motor:p.motor,
rx:p.rx,
usarOffset:!!p.usarOffset,
offsetAplicado:p.offsetAplicado||0,

jogada:jogadaDeSnapshot(p)
}
};
}
...
estado.timelines.AUTO=r.lista;
estado.freezes.AUTO=r.freeze;
...
estado.timelines.NORMAL[rx]=r.lista;
estado.freezes.NORMAL[rx]=r.freeze;
...
estado.timelines.DINAMICO[rx]=r.lista;
estado.freezes.DINAMICO[rx]=r.freeze;
...
if(!melhor||Math.abs(delta)<Math.abs(melhor.delta))melhor=candidato;
}
...
if(estado.modo==="AUTO")return estado.freezes.AUTO;

if(estado.motorManual==="DINAMICO")
return estado.freezes.DINAMICO[estado.manualRX];

return estado.freezes.NORMAL[estado.manualRX];
}
...
function atualizarVisualPrincipal(numero,configAntes,eraG1){
if(eraG1&&duplasVisual.length){
const ultima=duplasVisual[duplasVisual.length-1];
ultima.g1=numero;
ultima.fase="FINALIZADO";

if(jogadaCongelada&&jogadaCongelada.jogada){
const r=classificarJogada(numero,jogadaCongelada.jogada);
...
if(configAntes&&configAntes.valido&&configAntes.jogada){
resultadoAtivo=classificarJogada(numero,configAntes.jogada);
}
...
green:resultadoAtivo?resultadoAtivo.green:null,

fase:
resultadoAtivo&&resultadoAtivo.green===false
?"ESPERA_G1"
:"FINALIZADO",

motor:configAntes?configAntes.motor:null,
rx:configAntes?configAntes.rx:null
};

duplasVisual.push(entrada);
duplasVisual=duplasVisual.slice(-14);

if(resultadoAtivo&&resultadoAtivo.green===false){
jogadaCongelada=copiarJogada(configAntes);
}else{
jogadaCongelada=null;
}
...
const baseAntes=historico.slice(-35);
const seisAntes=calcularSeis(baseAntes);
const autoAntes=escolherAuto(seisAntes);
const ativaAntes=configAtiva(seisAntes,autoAntes);

const freezeVisivel=freezeSelecionado();
...
const dinamicaAtual=melhorDinamico(seisAntes);

const dinamicaAntes=
(
dinamicaAtual&&
estado.freezes.DINAMICO[dinamicaAtual.rx]
)
?
estado.freezes.DINAMICO[dinamicaAtual.rx]
:
...
atualizarOffsetGlobal(numero,dinamicaAntes);
...
const encontrados=texto.match(/\b(?:[0-9]|[12][0-9]|3[0-6])\b/g);
if(!encontrados)return[];
return encontrados.map(Number).slice(-35);
}

function resetarEstadoAnalise(){
estado.timelines=timelinesVazias();
estado.pendentes=pendentesVazios();
estado.freezes=freezesVazios();
...
<div class="painel">
<textarea id="entradaHistorico" placeholder="Cole o histórico — últimos 35"></textarea>
...
<div class="seletorTitulo">AUTO</div>
<div class="modos">
...
<div class="seletorGrupo">
<div class="seletorTitulo">NORMAL</div>
<div class="modos">
<button id="btnN4" class="modo">RX4</button>
<button id="btnN5" class="modo">RX5</button>
<button id="btnN6" class="modo">RX6</button>
</div>
</div>
...
<div class="modos">
<button id="btnD4" class="modo dinamico">RX4</button>
<button id="btnD5" class="modo dinamico">RX5</button>
<button id="btnD6" class="modo dinamico">RX6</button>
</div>
</div>

<div id="origemAuto" class="origemAuto">AUTO</div>
...
<div class="timelineSecao">
<div class="timelineTituloGrupo">MOTOR NORMAL</div>
...
document.getElementById("btnAUTO").onclick=selecionarAuto;

RX_LIST.forEach(rx=>{
document.getElementById("btnN"+rx).onclick=()=>selecionarManual("NORMAL",rx);
document.getElementById("btnD"+rx).onclick=()=>selecionarManual("DINAMICO",rx);
});
...
/* ============================================================
   LINHA DO TEMPO
   CADA LINHA USA O CAMPO fase REAL DELA.
============================================================ */

function renderTimeline(id,nome,lista){
const visual=(lista||[]).slice(-20);

const validos=visual.filter(x=>!x.semJogada);
const greens=validos.filter(x=>x.green===true).length;
const taxa=validos.length?greens/validos.length*100:0;
...
/* RESULTADO FOI O G1 DA JOGADA CONGELADA */
if(x.fase==="G1"){
if(x.green){
return'<span class="gl g1">G1</span>';
}
return'<span class="gl g1loss">L</span>';
}
...
'<div class="trioTitulo">TERMINAIS DO MOMENTO • 1 VIZINHO DE CADA LADO</div>'+
'<div class="trioLinha">'+

itens.map((x,i)=>
'<div class="terminal '+(i===0?"primeiro":(i===1?"segundo":"terceiro"))+'">'+
x.terminal+
'<small>FORÇA '+x.score.toFixed(1)+'</small>'+
'</div>'
).join("")+
...
const ranking=momento.duziasFisicas.ranking;

document.getElementById("duzias").innerHTML=
'<div class="trioTitulo">CONCENTRAÇÃO FÍSICA DAS DÚZIAS</div>'+
'<div class="duziasLinha">'+

ranking.map((x,i)=>
'<div class="duziaCard '+(i===0?"forte":"")+'">'+
'<small>'+x.duzia+'ª DÚZIA</small>'+
'<strong>'+(x.score*100).toFixed(0)+'%</strong>'+
'<span>'+x.contagem+'/14 • FÍSICA '+(x.fisica*100).toFixed(0)+'%</span>'+
'</div>'
).join("")+
...
const ordem={TIERS:0,ORPHELINS:1,ZERO:2,VOISINS:3};
return ordem[nome]!==undefined?ordem[nome]:99;
}

function htmlBlocoJogada(b,um=false){
return(
'<div class="bloco '+(um?"um":"")+'">'+
'<small>'+(um?"1 VIZINHO DO":"2 VIZINHOS DO")+'</small>'+
...
area.innerHTML='<div style="color:#777">AGUARDANDO DADOS</div>';
return;
}
...
todos.sort((a,b)=>{
const ra=ordemRegiao(a.regiao);
const rb=ordemRegiao(b.regiao);

if(ra!==rb)return ra-rb;
return indice(a.bloco.centro)-indice(b.bloco.centro);
});

const grupos=[];

todos.forEach(item=>{
let grupo=grupos.find(g=>g.nome===item.regiao);

if(!grupo){
grupo={nome:item.regiao,itens:[]};
grupos.push(grupo);
}
...
area.innerHTML=
'<div class="jogadaStatus '+(congelada?"congelada":"")+'">'+
(congelada?"JOGADA CONGELADA • G1":"JOGADA ATUAL")+
' • '+(config.motor==="DINAMICO"?"DINÂMICO":"NORMAL")+
' • RX'+config.rx+
'</div>'+

grupos.map(grupo=>
'<div class="grupoRegiao">'+
'<div class="grupoRegiaoTitulo">'+grupo.nome+'</div>'+
'<div class="jogada">'+
grupo.itens.map(item=>htmlBlocoJogada(item.bloco,item.um)).join("")+
'</div>'+
'</div>'
).join("");
}
...
classe="sem";
texto="—";
}else if(d.g1!==null&&d.g1!==undefined){
if(d.g1Green===true){
classe="g1";
texto="G1";
}else{
...
'<div class="bolaDupla" style="background:'+corRoleta(d.resultado)+'">'+
d.resultado+
'</div>'+
'<div class="resultadoEntrada '+classe+'">'+
texto+
(d.g1!==null&&d.g1!==undefined?' • '+d.g1:'')+
'</div>'+
'</div>'
);
}).join("");
...
if(!config||!config.valido)return"—";
if(config.motor!=="DINAMICO")return"NORMAL";

const o=config.offsetAplicado||0;

if(o===-2)return"V2 ESQUERDA • TODOS -2";
if(o===-1)return"V1 ESQUERDA • TODOS -1";
if(o===1)return"V1 DIREITA • TODOS +1";
if(o===2)return"V2 DIREITA • TODOS +2";
...
const momento=analisarMomento(base);
const seis=calcularSeis(base);
const auto=escolherAuto(seis);

garantirPendentes(seis,auto);

const ativa=configAtiva(seis,auto);
const freeze=freezeSelecionado();
...
if(estado.modo==="AUTO"){
document.getElementById("btnAUTO").classList.add("ativo");
}else{
const id=
...
document.getElementById("origemAuto").textContent=
auto
?(
"AUTO ESCOLHEU • "+
(auto.motor==="DINAMICO"?"DINÂMICO":"NORMAL")+
" • RX"+
auto.rx
)
:"AUTO • AGUARDANDO";

const liveAuto=statsTimeline(estado.timelines.AUTO);

document.getElementById("resumo").innerHTML=
'<div class="card">'+
'<small>BT10 AUTO</small>'+
'<strong>'+(auto?auto.backtest.taxa10.toFixed(0)+"%":"—")+'</strong>'+
'</div>'+

'<div class="card">'+
'<small>BT20 AUTO</small>'+
'<strong>'+(auto?auto.backtest.taxa20.toFixed(0)+"%":"—")+'</strong>'+
'</div>'+

'<div class="card">'+
'<small>REAL10 AUTO</small>'+
'<strong>'+(liveAuto.total?liveAuto.taxa10.toFixed(0)+"%":"—")+'</strong>'+
'</div>'+

'<div class="card">'+
'<small>REAL20 AUTO</small>'+
'<strong>'+(liveAuto.total?liveAuto.taxa20.toFixed(0)+"%":"—")+'</strong>'+
'</div>';

renderTrio(momento);
renderDuzias(momento);

renderTimeline("timelineAUTO","AUTO",estado.timelines.AUTO);

renderTimeline("timelineN4","RX4",estado.timelines.NORMAL[4]);
renderTimeline("timelineN5","RX5",estado.timelines.NORMAL[5]);
renderTimeline("timelineN6","RX6",estado.timelines.NORMAL[6]);

renderTimeline("timelineD4","RX4",estado.timelines.DINAMICO[4]);
renderTimeline("timelineD5","RX5",estado.timelines.DINAMICO[5]);
renderTimeline("timelineD6","RX6",estado.timelines.DINAMICO[6]);
...
renderJogada(
configExibida,
!!jogadaCongelada||!!freeze
);

const df=momento.duziasFisicas.dominante;

document.getElementById("status").textContent=
base.length+
"/35 • MOMENTO 14 • 35×14 ATIVO"+
(df?" • CONCENTRAÇÃO: "+df.duzia+"ª DÚZIA":"")+
" • CENTROS: "+
