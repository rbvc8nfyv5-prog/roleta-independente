(function () {
  ...
  btnConj.onclick=()=>{
    ...
    filtrosConjuntos.forEach(t=>analises.MANUAL.filtros.add(t));
  ...
  for(let t=0;t<=9;t++){
    ...
    b.onclick=()=>{
      ...
      if(modoConjuntos){
        ...
        analises.MANUAL.filtros.clear();
        filtrosConjuntos.forEach(x=>analises.MANUAL.filtros.add(x));
      } else {
  ...
  function render(){
    ...
    document.querySelectorAll(".modo,.auto,#btnConj").forEach(b=>{
      const ativo =
        b.dataset?.m===modoAtivo ||
        +b.dataset?.a===autoTAtivo ||
        (b.id==="btnConj" && modoConjuntos);
